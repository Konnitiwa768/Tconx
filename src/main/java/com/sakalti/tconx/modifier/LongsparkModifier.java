...
public class LongsparkModifier extends Modifier {
    ...
    public void afterEntityHit(ToolStack tool, int level, LivingEntity target, LivingEntity attacker, float damage, boolean isCritical) {
        ...
        LivingEntity entity6m = findEntityInFront(attacker, 6.0);
        if (entity6m != null) {
            float extraDamage = (float)(2.15 * (1.6 + level));
            entity6m.hurt(attacker.damageSources().playerAttack((net.minecraft.world.entity.player.Player)attacker), extraDamage);
        }

        LivingEntity entity8m = findEntityInFront(attacker, 8.0);
        if (entity8m != null) {
            float extraDamage = (float)(3.15 * (1.6 + level));
            entity8m.hurt(attacker.damageSources().playerAttack((net.minecraft.world.entity.player.Player)attacker), extraDamage);
        }

        LivingEntity entity10m = findEntityInFront(attacker, 10.0);
        if (entity10m != null) {
            float extraDamage = (float)(5.15 * (2.0 + level));
            entity10m.hurt(attacker.damageSources().playerAttack((net.minecraft.world.entity.player.Player)attacker), extraDamage);
        }
    }
}
